#include<stdio.h>
#include<stdlib.h>
#include<conio.h>

typedef struct student
{
    int rno;
    char name[20];
    struct subject
    {
        int scode;
        int sname[20];
        int mark;
    }sub[5];
    int total;
    float per;
}student;

void create()
{
    student *s;
    FILE *fp;
    int n, i, j;
    printf("\nEnter how many students you want to insert: ");
    scanf("%d",&n);
    s = (student*)calloc(n, sizeof(student));
    fp = fopen("mystudent.txt","w+");

    for(i=0;i<n;i++)
    {
        s[i].total = 0;
        s[i].per = 0;

        printf("\nEnter Roll no: ");
        scanf("%d",&s[i].rno);
        fflush(stdin);
        printf("\nEnter Name: ");
        scanf("%s",s[i].name);
        for(j=0;j<3;j++)
        {
            printf("\nEnter marks of subject %d : ",j+1);
            scanf("%d",&s[i].sub[j].mark);
            s[i].total += s[i].sub[j].mark;
        }
        s[i].per = s[i].total/3.0;
        fwrite(&s[i], sizeof(student),1,fp);
    }
    fclose(fp);
}

void display()
{
    student s1;
    FILE *fp;
    int j;
    fp = fopen("mystudent.txt","r");
    while(fread(&s1, sizeof(student),1,fp))
    {
        printf("\n%-5d%-20s",s1.rno,s1.name);
        for(j=0;j<3;j++)
        {
            printf("%4d",s1.sub[j].mark);
        }
        printf("%5d%7.2f",s1.total,s1.per);
    }
    fclose(fp);
}

void append()
{
 student *s;
    FILE *fp;
    int n, i, j;
    printf("\nEnter how many students you want to insert: ");
    scanf("%d",&n);

    s = (student*)calloc(n, sizeof(student));
    fp = fopen("mystudent.txt","w+");


    for(i=0;i<n;i++)
    {
        s[i].total = 0;
        s[i].per = 0;

        printf("\nEnter Roll no: ");
        scanf("%d",&s[i].rno);
        fflush(stdin);
        printf("\nEnter Name: ");
        scanf("%s",s[i].name);
        for(j=0;j<3;j++)
        {
            printf("\nEnter marks of subject%d : ",j+1);
            scanf("%d",&s[i].sub[j].mark);
            s[i].total += s[i].sub[j].mark;
        }
        s[i].per = s[i].total/3.0;
        fwrite(&s[i], sizeof(student),1,fp);
    }
    fclose(fp);
}

void noofrecords()
{
    student s1;
    FILE*fp;
    fp=fopen("mystudent.txt","r");
    fseek(fp,0,SEEK_END);
    int n = ftell(fp)/sizeof(student);
    printf("\n\n NUMBER OF RECORDS=%d",n);
    fclose(fp);
}

void serch()
{
    student s1;
    FILE *fp;
    int j,rno,found=0;
    fp = fopen("mystudent.txt","r");
    printf("\nenter the roll number to serch:",rno);
    scanf("%d",&rno);
    while(fread(&s1, sizeof(student),1,fp))
    {
        if(s1.rno==rno)
        {
        found=1;
        printf("\n%-5d%-20s",s1.rno,s1.name);
        for(j=0;j<3;j++)
        {
            printf("%4d",s1.sub[j].mark);
        }
        printf("%5d%7.2f",s1.total,s1.per);
        }
    }
    if(!found)
    printf("\nrecord is not found\n");
    fclose(fp);
}

int main()
{
    int ch;
    do
    {
        printf("\n\n1. CREATE");
        printf("\n2. DISPLAY");
        printf("\n3. APPEND");
        printf("\n4. NO OF RECORDS");
        printf("\n5. SERCH");
        printf("\n0. EXIT");

        printf("\nEnter your choice (1-5): ");
        scanf("%d",&ch);

        switch(ch)
        {
            case 1:
                create();
            break;
            case 2:
                display();
            break;
            case 3:
                append();
            break;
             case 4:
                noofrecords();
            break;
             case 5:
               serch();
            break;
        }
    }
    while(ch!=0);

    return 0;
}
// use the file for many more cases in this program it is used for only 5 cases